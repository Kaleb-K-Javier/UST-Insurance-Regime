###############################################################################
# Mid‑Continent Casualty ‑ Premium Engine  (Texas UST data)
#   1. create static‑tank attribute table  ➜  Outputs/texas_static_tank_details.csv
#   2. apply Mid‑Continent OK‑SERFF factors (2006‑11) to all TX facilities
#      whose FR issuer is Mid‑Continent
#   3. output tank‑month and facility‑year premia
###############################################################################
...
static_tank_details <- ust_raw[, .(
  FACILITY_ID, UST_ID, INSTALL_DATE, CLOSED_DATE, tank_status, is_closed_removed,
  # construction
  is_steel_cathodic, is_double_walled_steel,
...
###############################################################################
##  SECTION B  –  Mid‑Continent OK‑SERFF premium engine (2006‑01 … 2014‑04) ##
###############################################################################

WINDOW_START <- as.Date("2006-01-01")
WINDOW_END   <- as.Date("2014-04-30")
...
## ---- 2) Age factor ---------------------------------------------------------
age_factor <- function(age_years) {
  fcase(
    is.na(age_years), NA_real_,      # keep NA so we can drop unrated rows later
    age_years <=  5, -0.10,
    age_years <= 10,  0.00,
    age_years <= 15,  0.05,
    age_years <= 20,  0.10,
...
), by = .(FACILITY_ID, MONTH, YEAR)][YEAR >= 2006 & YEAR <= 2014]
...
uniqueN(fac_year$FACILITY_ID)
uniqueN(fa_contracts$FACILITY_ID)
...
fwrite(ust_tm,
  file.path("Rate FIllings", "Mid-Continent Casualty Company ­– 23418", "texas_midcontinent_tank_month_premium_2006_to_04_2014.csv"))
fwrite(fac_year,
  file.path("Rate FIllings", "Mid-Continent Casualty Company ­– 23418", "texas_midcontinent_facility_year_premium_2006_to_04_2014.csv"))