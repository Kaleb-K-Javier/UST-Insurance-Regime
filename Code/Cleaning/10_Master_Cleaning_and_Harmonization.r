...
#   2. Applies Winning ID Logic (AL, MI, TX, etc.) to merge EPA Lat/Long.
...
# Extended Type Casting (Include indicators for analysis)
...
# Ensure columns exist before casting (fill with NA if missing from source)
...
# Standardize Status to 'Closed' or 'Open' for logic checks
...
# --- SURGICAL INSERTION: Standardize Coordinates (Mercator -> Lat/Long) ---
message("\n--- Step 1.5: Standardizing Coordinate Systems ---")

if(requireNamespace("sf", quietly = TRUE)) {
  # 1. Identify rows with coordinates in meters (Web Mercator)
  # Heuristic: Latitude > 90 is impossible for degrees.
  # We check 'latitude' because 'latitude_final' hasn't been created yet.
  mercator_idx <- which(
  ...
  if(length(mercator_idx) > 0) {
    message(paste("Detected", length(mercator_idx), "rows with Projected Coordinates (likely Web Mercator). Converting to Degrees..."))
    
    # 2. Create subset for conversion
    # We use a temporary data.table to avoid copying the whole master 
    sub_geo <- master_tanks[mercator_idx, .(latitude, longitude)]
    
    # 3. Convert using sf
    # Input CRS: 3857 (Web Mercator - units in meters)
    # Output CRS: 4326 (WGS84 - units in degrees)
    sf_obj   <- sf::st_as_sf(sub_geo, coords = c("longitude", "latitude"), crs = 3857)
    sf_trans <- sf::st_transform(sf_obj, crs = 4326)
    
    # 4. Extract new coordinates
    coords_new <- sf::st_coordinates(sf_trans)
    
    # 5. Update Master Table by reference
  ...
  warning("Package 'sf' is missing. Skipping coordinate standardization. Expect mapping errors for AK, CA, AZ, etc.")
}
...
if(file.exists(epa_path)) {
  ...
  epa_lookup <- unique(epa_dt[!is.na(lat_epa) & !is.na(long_epa), 
                              .(merge_id = epa_id, lat_epa=as.numeric(lat_epa), long_epa=as.numeric(long_epa))], by="merge_id")
...
# --- A. Preparation & Standardization ---

# 1. Enforce Character Type for FIPS (Preserve '01001')
...
# 2. Define Cleaning Function (Handles LA Parishes, AK Boroughs, Standard Counties)
clean_county_name <- function(x) {
  x <- toupper(trimws(x))
  # Remove suffixes to match reference standard
  str_remove(x, "\\s+(COUNTY|PARISH|BOROUGH|CENSUS AREA)$")
}

# 3. Load References
has_zip_ref <- file.exists(here("Data", "Raw", "uszips.csv"))
has_sf      <- requireNamespace("sf", quietly = TRUE) && requireNamespace("tigris", quietly = TRUE)

# --- B. Build Lookup Tables ---

# Ref 1: Name-to-FIPS Lookup (using tigris fips_codes)
if(has_sf) {
  data("fips_codes", package = "tigris", envir = environment())
  setDT(fips_codes)
  
  # Create Lookup: State + Clean Name -> FIPS
...
# --- C. EXECUTE CASCADE (Fill only if missing) ---

# 0. Pre-process Master Names
# We create a temp clean column to match against our lookup
master_tanks[, temp_clean_name := clean_county_name(county_name)]

# PASS 1: Name Match (If we have Name, get FIPS)
if(has_sf) {
  message("Pass 1: Filling FIPS via County Name match...")
  
  # Join and update FIPS only where currently missing
...
# PASS 2: Zip Match (If missing Name/FIPS, use Zip)
if(has_zip_ref) {
  ...
  # Conditional Fill: Only overwrite if currently NA
...
# PASS 3: Spatial Match (If still missing, use Lat/Long)
# Targets: Rows that are STILL missing FIPS (primary key) and have Geo
...
if(length(missing_spatial) > 0 && has_sf) {
  ...
  us_sf <- us_sf[, c("GEOID", "NAME")] # GEOID is FIPS
...
# Define Groups
tier1_controls <- c("AL", "AR", "CO", "LA", "ME", "NM", "OK", "PA", "TN")
tier2_controls <- c("ID", "KS", "KY", "MD", "MA", "MN", "NC", "ND", "OH", "VA")
treated_states <- c("AZ", "CT", "FL", "IA", "MI", "NJ", "WI")
...
# A. Calculate LUST Date Missingness
...
# Print Summary (Focus on critical columns for review)
print(report)
# 5. Save -----------------------------------------------------------------
