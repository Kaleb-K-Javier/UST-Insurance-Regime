...
fwrite(static_tank_details,
       get_data_path( "texas_static_tank_details_2021.csv"))
...
RATING_START <- 2021                    # inclusive
RATING_END   <- 2030L                    # leave roomy for future years
...
status_factor <- function(status_chr){
  fcase(
    status_chr == "In Use",                  0.00,      # table I‑a
    status_chr == "Temporarily Out of Use", -0.25,      # table I‑b
    ## The 2021 filing is silent on permanently closed tanks; keep the old debit
    status_chr == "Permanently Out of Use", -0.75,
    default = NA_real_                                    # drop if unknown
  )
}


## ---------- 3) ILF still applies EXACTLY as before -------------------------
## (keep your existing `ilf_factor()` definition – nothing changes)
...
WINDOW_START <- ymd("2021-05-01")
WINDOW_END   <- ymd("2030-12-31")
...
## ────────────────────────────────────────────────────────────────────────────
tank_static <- fread( get_data_path("texas_static_tank_details_2021.csv") )[
...
ust_tm[, site_capacity_gal := sum(CAPACITY*(is_closed_removed==0), na.rm = TRUE), by = .(FACILITY_ID, YEAR, MONTH)]

## ── Secondary‑factor columns ------------------------------------------------
ust_tm[ , `:=`(
   status_load        =status_factor(tank_status), 
...
## ---------- updated Tank‑status factor  (TOP manual ‑ 05/2021) --------------
...
fwrite(fac_year,
   file.path("Rate FIllings", "Mid-Continent Casualty Company ­– 23418",
           "texas_midcontinent_facility_year_premium_2021_onwards.csv"))

cat("✔ Finished rating Mid‑Continent facilities for contracts effective ≥ 2019‑01‑01\n")
