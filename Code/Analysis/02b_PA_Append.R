...
# Ticket 050 PART A -- extends the same machinery to NM (unknown-wall).
#
# WHAT THIS DOES: loads Data/Analysis/panel_dt.csv + facility_panel.csv AS-IS
# (study-state rows are never recomputed), builds PA and NM (PRICING/HAZARD-
# eligible only, NOT part of the closure-DiD/structural sample) from
# master_tanks (Data/Processed/Master_Harmonized_UST_Tanks.csv) + each
# state's own harmonized LUST source (NOT Master_Harmonized_LUST.csv -- see
# the T046.0 comment below for why), unions PA+NM onto the loaded panels,
# and OVERWRITES the canonical panel_dt.csv / facility_panel.csv.
#
# RUN ORDER (D5): 02b_Tank_level_Panel_Build.R (rare, base-data changes) THEN
# this script (fast, adds PA+NM). This script's output OVERWRITES the
# canonical panel; a future full 02b run drops PA/NM until this script is
# re-run again.
#
# SCOPE: PA (ticket 049) + NM (ticket 050 PART A). NM has NO tank wall type
# (D1/D2, ticket 050): make_model_tank keeps "Unknown-Wall" as a real level
# instead of NA'ing it out (done ONLY in the NM branch below -- the PA branch
# and the main 02b make_model logic are untouched); has_unknown_wall falls
# out of the existing facility-aggregation formulas automatically once NM
# tanks carry mm_wall=="Unknown-Wall".
#
...
# type-safe union), lifted here as a standalone append, then extended to NM.
################################################################################
...
# States built by this script: PA (ticket 049) + NM (ticket 050 PART A).
# Kept as separate per-state constants so each branch's own filters stay
# state-specific (the NM make_model rule differs from PA's -- D1), with
# PRICING_STATES as the umbrella set used by the idempotency guard and the
# D-ACCEPT non-pricing-state check.
PA_STATES      <- c("PA")
NM_STATES      <- c("NM")
PRICING_STATES <- c(PA_STATES, NM_STATES)
...
# Idempotency guard: drop any pre-existing PA/NM rows from a loaded main
# panel before appending, so re-running this script never double-appends.
# Handles the flag column not existing yet (true on a panel that predates
# this script's first run).
guard_drop_prior_rows <- function(dt, dt_name) {
  has_role  <- "panel_state_role" %in% names(dt)
  drop_idx  <- dt$state %in% PRICING_STATES
  if (has_role) {
...
#### STEP 2A: BUILD PA (lifts T046.1-T046.13 from commit a4014ce) ####

cat("========================================\n")
cat("STEP 2A: BUILD PA ROWS\n")
cat("========================================\n\n")
...
pa_tanks <- master_tanks[state %in% PA_STATES & !is.na(tank_installed_date)]
...
# Master_Harmonized_LUST.csv -- see T046.0 comment), filtered to PA_STATES
# defensively (the source file is PA-only already).
cat("T046.12: PA leak history...\n")

pa_leaks_fac_yr <- pa_lust_raw[
  !is.na(report_date) & state %in% PA_STATES,
  .(
    n_leak_reports   = .N,
...
#### STEP 2B: BUILD NM ROWS (ticket 050 PART A) ####
# Mirrors T046.1-T046.13 above (PA), with the D1/D2 deltas:
#   - make_model_tank/_noage/_3dim keep "Unknown-Wall" as the wall token
#     instead of NA'ing it out -- this is the ONLY logic delta from the PA
#     branch's cell-formation rule, and it is scoped to this NM block only
#     (pa_tanks / study_tanks / the main 02b make_model logic are untouched).
#   - has_single_walled/has_double_walled/has_unknown_wall need no special
#     handling: they already fall out of the facility-aggregation formulas
#     lifted from PA (as.integer(n_unk_wall_active > 0) etc.), since every
#     NM tank carries mm_wall == "Unknown-Wall".
#   - Leak source is NM's own harmonized file, not Master_Harmonized_LUST.csv
#     (unlike PA, NM's rows are NOT missing from the master file -- verified
#     2026-07-02 -- but the state-specific file is used anyway for
#     consistency with the PA branch and to not depend on 10_Master's
#     pricing-state handling).

cat("========================================\n")
cat("STEP 2B: BUILD NM ROWS\n")
cat("========================================\n\n")

#### NM.0 NM leak source (state-specific harmonized file) ####
cat("NM.0: Load NM LUST source (NM_Harmonized_LUST.csv)...\n")

nm_lust_raw <- fread(
  here("Data", "Raw", "state_databases", "New Mexico", "NM_Harmonized_LUST.csv"),
  na.strings = c("", "NA", "N/A")
)
nm_lust_raw[, `:=`(
  facility_id = toupper(trimws(as.character(facility_id))),
...
nm_lust_raw[, panel_id    := paste(facility_id, state, sep = "_")]
nm_lust_raw[, report_date := as.IDate(report_date)]

log_step(sprintf("Loaded NM LUST source: %s rows, %d cols",
  fmt_n(nrow(nm_lust_raw)), ncol(nm_lust_raw)))
cat("\n")

#### NM.1 NM tank source (MIRRORS T046.1) ####
cat("NM.1: NM tank source...\n")

nm_tanks <- master_tanks[state %in% NM_STATES & !is.na(tank_installed_date)]
# texas_treated is a formula-safe input only (did_term = texas_treated *
# post_1999), NOT a DiD-eligibility signal -- same reasoning as the PA branch.
nm_tanks[, texas_treated := 0L]

log_step(sprintf("NM tanks: %s", fmt_n(nrow(nm_tanks))))

# PRE-CHECK (ticket 050 PART A step 2): confirm mm_fuel/mm_capacity/
# mm_install_cohort are populated for NM -- only wall is expected missing.
# If fuel or cohort are ALSO largely unknown, make_model can't form even
# with the Unknown-Wall token, and the design needs revisiting (architect
# call, not this script's to improvise) -- STOP rather than silently
# building a mostly-empty NM sample.
nm_pct_fuel_known   <- 100 * mean(nm_tanks$mm_fuel != "Unknown-Fuel", na.rm = TRUE)
nm_pct_cohort_known <- 100 * mean(!is.na(nm_tanks$mm_install_cohort))
nm_pct_cap_known    <- 100 * mean(!is.na(nm_tanks$mm_capacity) & nm_tanks$mm_capacity != "", na.rm = TRUE)
log_step(sprintf(
  "NM completeness pre-check: fuel known %.1f%% | capacity known %.1f%% | install_cohort known %.1f%%",
  nm_pct_fuel_known, nm_pct_cap_known, nm_pct_cohort_known))
if (nm_pct_fuel_known < 50 || nm_pct_cohort_known < 50) {
  stop(sprintf(paste(
    "NM completeness pre-check FAILED (ticket 050 PART A step 2): fuel known",
    "%.1f%%, install_cohort known %.1f%%. make_model_tank cannot form even",
    "with the Unknown-Wall token if fuel/cohort are this sparse. This is an",
    "architect design call (coarser NM make_model, or wall-only pricing for",
    "NM), not something to improvise here. STOP."),
    nm_pct_fuel_known, nm_pct_cohort_known))
}

#### NM.2 NM annual tank-year panel (MIRRORS T046.2, D1 delta on make_model) ####
cat("NM.2: NM annual tank-year panel...\n")

nm_tanks[, release_det_deadline_yr := fcase(
  install_yr_int <= 1964L,         RELEASE_DET_DEADLINE_YR$pre_1965,
  install_yr_int %in% 1965L:1969L, RELEASE_DET_DEADLINE_YR$y1965_69,
  install_yr_int %in% 1970L:1974L, RELEASE_DET_DEADLINE_YR$y1970_74,
  install_yr_int %in% 1975L:1979L, RELEASE_DET_DEADLINE_YR$y1975_79,
...
nm_tanks[, make_model_tank  := paste(mm_wall, mm_fuel, mm_capacity, mm_install_cohort, sep = "|")]
nm_tanks[, make_model_noage := paste(mm_wall, mm_fuel, mm_capacity, sep = "|")]
nm_tanks[, make_model_3dim  := paste(mm_wall, mm_fuel, mm_install_cohort, sep = "|")]
# D1: unlike the PA branch, do NOT NA out on mm_wall == "Unknown-Wall" -- NM
# has no other wall value, so that condition would drop every NM tank. Only
# unknown fuel / missing cohort NA the cell (same as the PA branch otherwise).
nm_tanks[
  mm_fuel == "Unknown-Fuel" | is.na(mm_install_cohort),
  `:=`(make_model_tank  = NA_character_,
       make_model_noage = NA_character_,
       make_model_3dim  = NA_character_)
]
...
setorder(nm_ty_panel, tank_panel_id, panel_year)

nm_ty_panel[, first_year_churn := as.integer(
  panel_year == install_yr_int & closure_event == 1L
)]

log_step(sprintf("NM panel: %s rows | %s tanks | %s facilities | %s first-year-churn flagged",
  fmt_n(nrow(nm_ty_panel)),
  fmt_n(uniqueN(nm_ty_panel$tank_panel_id)),
  fmt_n(uniqueN(nm_ty_panel$panel_id)),
  fmt_n(sum(nm_ty_panel$first_year_churn))))

#### NM.3 NM mandate controls (MIRRORS T046.3) ####
cat("NM.3: NM mandate controls...\n")
...
nm_ty_panel[, mandate_release_det := as.integer(
  !is.na(release_det_deadline_yr) &
  panel_year >= 1989L &
  panel_year <= release_det_deadline_yr
)]
nm_ty_panel[, mandate_spill_overfill := as.integer(
  !is.na(release_det_deadline_yr) &
  panel_year %in% 1992L:1994L
)]
nm_ty_panel[, mandate_integrity := as.integer(
  !is.na(release_det_deadline_yr) &
  panel_year %in% 1996L:1998L
)]

nm_mandate_post_check <- nm_ty_panel[
  panel_year >= 1999L,
  .(n_release_det    = sum(mandate_release_det),
    n_spill_overfill = sum(mandate_spill_overfill),
    n_integrity      = sum(mandate_integrity))
...
stopifnot(
  "NM mandate_release_det nonzero post-1998"    = nm_mandate_post_check$n_release_det    == 0L,
  "NM mandate_spill_overfill nonzero post-1998" = nm_mandate_post_check$n_spill_overfill == 0L,
  "NM mandate_integrity nonzero post-1998"      = nm_mandate_post_check$n_integrity      == 0L
)
log_step("NM mandate overlap check passed.")

nm_ty_panel[, rel_year_bin := fcase(
  rel_year <= -14L, -14L,
  rel_year >=  16L,  16L,
  default = as.integer(rel_year)
)]

#### NM.4 NM panel_dt rows (MIRRORS T046.4) ####
cat("NM.4: NM panel_dt rows...\n")

nm_panel_dt <- nm_ty_panel[
  !is.na(make_model_tank) &
  !is.na(did_term)        &
  tstop > tstart
]
log_step(sprintf("NM panel_dt: %s tank-years | %s tanks",
  fmt_n(nrow(nm_panel_dt)), fmt_n(uniqueN(nm_panel_dt$tank_panel_id))))

#### NM.5 NM churn-filtered facility-year stock (MIRRORS T046.5) ####
cat("NM.5: NM facility-year stock...\n")

nm_ty_panel_clean <- nm_ty_panel[first_year_churn == 0L | is.na(first_year_churn)]
nm_tanks_no_churn <- nm_tanks[
  is.na(tank_closed_date) | year(tank_installed_date) != year(tank_closed_date)
]

nm_fac_stock <- nm_ty_panel_clean[
  ,
  .(
    n_tanks_active        = .N,
    n_sw_active           = sum(mm_wall == "Single-Walled", na.rm = TRUE),
...
log_step(sprintf("NM facility-year stock: %s rows | %s facilities",
  fmt_n(nrow(nm_fac_stock)), fmt_n(uniqueN(nm_fac_stock$panel_id))))

#### NM.6 NM installs (MIRRORS T046.6) ####
cat("NM.6: NM installs...\n")

nm_installs_by_fac_yr <- nm_tanks_no_churn[
  !is.na(tank_installed_date),
  .(
    n_installs         = .N,
    n_dw_installs      = sum(mm_wall == "Double-Walled", na.rm = TRUE),
...
nm_fac_stock <- merge(nm_fac_stock, nm_installs_by_fac_yr,
                      by = c("panel_id", "panel_year"), all.x = TRUE)
for (v in c("n_installs", "n_dw_installs", "n_sw_installs", "capacity_installed"))
  nm_fac_stock[is.na(get(v)), (v) := 0L]

#### NM.7 NM closure type classification (MIRRORS T046.7) ####
cat("NM.7: NM closure type classification...\n")

nm_fac_max_install <- nm_tanks[
  !is.na(tank_installed_date),
  .(fac_max_install_date = max(tank_installed_date, na.rm = TRUE)),
  by = panel_id
]
nm_closed_tanks_fac <- nm_tanks[
  !is.na(tank_closed_date) &
  year(tank_closed_date) != year(tank_installed_date),
  .(panel_id, tank_panel_id, tank_closed_date, tank_installed_date,
    mm_wall, mm_fuel, capacity)
]
nm_closed_tanks_fac <- merge(nm_closed_tanks_fac, nm_fac_max_install, by = "panel_id", all.x = TRUE)
nm_closed_tanks_fac[, is_replacement := as.integer(
  !is.na(fac_max_install_date) & fac_max_install_date > tank_closed_date
)]
nm_closed_tanks_fac[is.na(is_replacement), is_replacement := 0L]
nm_closed_tanks_fac[, is_sw_replacement := as.integer(
  is_replacement == 1L & mm_wall == "Single-Walled"
)]

nm_closure_type_agg <- nm_closed_tanks_fac[, .(
  n_closures_permanent   = sum(is_replacement == 0L),
  n_closures_replacement = sum(is_replacement == 1L),
  n_sw_replacement       = sum(is_sw_replacement, na.rm = TRUE),
  capacity_permanent     = sum(capacity[is_replacement == 0L], na.rm = TRUE),
...
nm_fac_stock <- merge(nm_fac_stock, nm_closure_type_agg,
                      by = c("panel_id", "panel_year"), all.x = TRUE)
for (v in c("n_closures_permanent", "n_closures_replacement", "n_sw_replacement",
            "capacity_permanent", "capacity_replacement"))
  nm_fac_stock[is.na(get(v)), (v) := 0L]
...
rm(nm_fac_max_install, nm_closed_tanks_fac, nm_closure_type_agg)

#### NM.8 NM YoY changes (MIRRORS T046.8) ####
cat("NM.8: NM YoY changes...\n")

setorder(nm_fac_stock, panel_id, panel_year)
nm_fac_stock[, `:=`(
  lag_n_tanks   = shift(n_tanks_eoy,           1L, type = "lag"),
  lag_n_sw      = shift(n_sw_eoy,              1L, type = "lag"),
  lag_n_dw      = shift(n_dw_eoy,              1L, type = "lag"),
...
#### NM.9 NM facility biography (MIRRORS T046.9) ####
cat("NM.9: NM facility biography...\n")
...
for (col in setdiff(names(nm_fac_bio), c("panel_id", "state"))) {
  if (is.numeric(nm_fac_bio[[col]])) {
    set(nm_fac_bio, which(!is.finite(nm_fac_bio[[col]])), col, NA)
  }
}

nm_fac_bio[, total_tanks_bin := fcase(
  total_tanks_ever == 1L,                 "1",
  total_tanks_ever %between% c(2L, 3L),   "2-3",
  total_tanks_ever %between% c(4L, 6L),   "4-6",
  total_tanks_ever %between% c(7L, 10L),  "7-10",
...
nm_fac_bio[, sw_share_ever_bin := fcase(
  prop_sw_ever == 1,    "All SW",
  prop_sw_ever >  0.5,  "Majority SW",
  prop_sw_ever <= 0.5,  "Minority/No SW",
  default = NA_character_
)]
nm_fac_bio[, fac_is_incumbent := as.integer(first_install_yr < 1999L)]
nm_fac_bio[, fac_vintage := fcase(
  first_install_yr <  1975L,                    "Pre-1975",
  first_install_yr %between% c(1975L, 1984L),   "1975-1984",
  first_install_yr %between% c(1985L, 1988L),   "1985-1988",
  first_install_yr %between% c(1989L, 1998L),   "1989-1998",
...
nm_fac_bio[, birth_cohort_bin := fcase(
  first_install_yr <= 1964L,                    "Pre-1965",
  first_install_yr %between% c(1965L, 1979L),   "1965-1979",
  first_install_yr %between% c(1980L, 1988L),   "1980-1988",
  first_install_yr %between% c(1989L, 1998L),   "1989-1998",
...
log_step(sprintf("NM biography: %s facilities", fmt_n(nrow(nm_fac_bio))))

#### NM.10 NM reform-date portfolio snapshot (MIRRORS T046.10) ####
cat("NM.10: NM reform-date portfolio snapshot...\n")

nm_reform_tank_ids <- nm_tanks[
  tank_installed_date <= REFORM_DATE &
  (is.na(tank_closed_date) | tank_closed_date > REFORM_DATE),
  tank_panel_id
]
nm_tanks_at_reform_fac <- nm_tanks[
  tank_panel_id %in% nm_reform_tank_ids,
  .(panel_id, tank_panel_id, mm_wall, mm_fuel, mm_capacity,
    capacity, install_yr_int,
    age_at_reform = as.numeric(REFORM_DATE - tank_installed_date) / 365.25)
]

nm_fac_mm_reform <- nm_tanks_at_reform_fac[, .(
  n_tanks_at_reform     = .N,
  n_sw_at_reform        = sum(mm_wall == "Single-Walled", na.rm = TRUE),
  n_dw_at_reform        = sum(mm_wall == "Double-Walled", na.rm = TRUE),
  total_capacity_reform = sum(capacity, na.rm = TRUE),
...
for (col in setdiff(names(nm_fac_mm_reform), "panel_id")) {
  if (is.numeric(nm_fac_mm_reform[[col]])) {
    set(nm_fac_mm_reform, which(!is.finite(nm_fac_mm_reform[[col]])), col, NA)
  }
}

nm_fac_mm_reform[, n_tanks_bin := fcase(
  n_tanks_at_reform == 1L,                   "1",
  n_tanks_at_reform %between% c(2L, 3L),     "2-3",
  n_tanks_at_reform %between% c(4L, 6L),     "4-6",
  n_tanks_at_reform %between% c(7L, 10L),    "7-10",
...
nm_fac_mm_reform[, n_sw_bin := fcase(
  n_sw_at_reform == 0L,                      "0",
  n_sw_at_reform == 1L,                      "1",
  n_sw_at_reform %between% c(2L, 3L),        "2-3",
  n_sw_at_reform >= 4L,                       "4+",
...
nm_fac_mm_reform[, capacity_bin := fcase(
  is.na(total_capacity_reform) | total_capacity_reform == 0, NA_character_,
  total_capacity_reform < 10000,                              "Under-10k",
  total_capacity_reform %between% c(10000, 29999),            "10k-30k",
  total_capacity_reform %between% c(30000, 59999),            "30k-60k",
...
nm_fac_mm_reform[, oldest_age_bin := fcase(
  oldest_age_reform <   5,                "0-4yr",
  oldest_age_reform %between% c( 5,  9),  "5-9yr",
  oldest_age_reform %between% c(10, 14),  "10-14yr",
  oldest_age_reform %between% c(15, 19),  "15-19yr",
...
nm_fac_mm_reform[, sw_share_bin := fcase(
  prop_sw == 1,    "All SW",
  prop_sw >  0.5,  "Majority SW",
  prop_sw <= 0.5,  "Minority/No SW",
  default = NA_character_
)]
nm_fac_mm_reform[, make_model_fac := fifelse(
  !is.na(oldest_age_bin) & fac_wall_reform != "Unknown",
  paste(fac_wall_reform, fac_fuel_reform, oldest_age_bin, sep = "|"),
  NA_character_
)]

log_step(sprintf("NM reform snapshot: %s facilities | identified cell: %s (NM is wall-Unknown at reform by construction -- D4, expected 0)",
  fmt_n(nrow(nm_fac_mm_reform)),
  fmt_n(nm_fac_mm_reform[!is.na(make_model_fac), .N])))

rm(nm_reform_tank_ids, nm_tanks_at_reform_fac)

#### NM.11 NM DiD time vars + exit/entry + cumulatives (MIRRORS T046.11) ####
cat("NM.11: NM time variables, exit/entry, cumulatives...\n")
...
setorder(nm_fac_stock, panel_id, panel_year)
nm_fac_stock[, last_panel_year  := max(panel_year), by = panel_id]
nm_fac_stock[, first_panel_year := min(panel_year), by = panel_id]
nm_fac_stock[, facility_exit := as.integer(
  panel_year == last_panel_year & last_panel_year < 2020L
)]
nm_fac_stock[, facility_complete_closure := as.integer(
  n_closures > 0 & n_tanks_eoy == 0L
)]
nm_fac_stock[, facility_entry := as.integer(
  panel_year == first_panel_year & first_panel_year > 1985L
)]
nm_fac_stock[, post_reform_entrant := as.integer(first_panel_year >= 1999L)]
nm_fac_stock[, years_in_panel := panel_year - first_panel_year]
...
log_step(sprintf("NM stock panel: %s rows | %s fac | %s exits | %s entries",
  fmt_n(nrow(nm_fac_stock)), fmt_n(uniqueN(nm_fac_stock$panel_id)),
  fmt_n(nm_fac_stock[facility_exit == 1L, .N]),
  fmt_n(nm_fac_stock[facility_entry == 1L, .N])))

#### NM.12 NM leak / leak-closure / leak-install pairing (MIRRORS T046.12) ####
cat("NM.12: NM leak history...\n")

nm_leaks_fac_yr <- nm_lust_raw[
  !is.na(report_date) & state %in% NM_STATES,
  .(
    n_leak_reports   = .N,
    n_leak_incidents = uniqueN(report_date),
    leak_year        = 1L
...
nm_fac_stock <- merge(nm_fac_stock, nm_leaks_fac_yr,
                      by = c("panel_id", "panel_year"), all.x = TRUE)
nm_fac_stock[is.na(n_leak_reports), `:=`(
  n_leak_reports = 0L, n_leak_incidents = 0L, leak_year = 0L
)]
nm_fac_stock[, n_leaks := n_leak_incidents]

n_nm_leak_fac_years <- nm_fac_stock[leak_year == 1L, .N]
log_step(sprintf("NM LUST: %s fac-years with leaks | %s reports | %s incidents",
  fmt_n(n_nm_leak_fac_years),
  fmt_n(nm_fac_stock[, sum(n_leak_reports)]),
  fmt_n(nm_fac_stock[, sum(n_leak_incidents)])))
if (n_nm_leak_fac_years == 0L) {
  stop("NM leak join produced 0 leak fac-years. Check that Data/Raw/state_databases/New Mexico/NM_Harmonized_LUST.csv has populated report_date values and that its facility_id joins to master_tanks' NM facility_id. STOP.")
}

nm_closures_for_lust <- nm_tanks[
  !is.na(tank_closed_date),
  .(panel_id, closure_date = tank_closed_date, tank_install_date = tank_installed_date)
]
nm_leaks_for_lust <- nm_lust_raw[
  !is.na(report_date) & state %in% NM_STATES,
  .(panel_id, report_date)
]
setkey(nm_closures_for_lust, panel_id)
setkey(nm_leaks_for_lust, panel_id)

nm_lust_pairs <- nm_leaks_for_lust[nm_closures_for_lust, on = .(panel_id), allow.cartesian = TRUE]
nm_lust_pairs <- nm_lust_pairs[is.na(tank_install_date) | report_date >= tank_install_date]

nm_lust_pairs[, diff := as.numeric(report_date - closure_date)]
nm_lust_pairs[, `:=`(
  is_rev_prim = diff >= 0    & diff <= 60,
  is_kno_prim = diff < -180,
  is_ind_prim = diff >= -180 & diff < 0,
...
nm_closure_lust_flags <- nm_lust_pairs[, .(
  has_rev_prim = any(is_rev_prim), has_kno_prim = any(is_kno_prim),
  has_ind_prim = any(is_ind_prim),
  has_rev_nar  = any(is_rev_nar),  has_kno_nar  = any(is_kno_nar),
  has_rev_wid  = any(is_rev_wid),  has_kno_wid  = any(is_kno_wid),
...
nm_closure_lust_flags[, panel_year := year(closure_date)]

nm_annual_lust_flags <- nm_closure_lust_flags[, .(
  tank_closure_revealed          = as.integer(any(has_rev_prim)),
  tank_closure_known_leak        = as.integer(any(has_kno_prim)),
  tank_closure_indeterminate     = as.integer(any(has_ind_prim)),
  tank_closure_revealed_narrow   = as.integer(any(has_rev_nar)),
...
nm_fac_stock <- merge(nm_fac_stock, nm_annual_lust_flags,
                      by = c("panel_id", "panel_year"), all.x = TRUE)
for (v in grep("^tank_closure_", names(nm_fac_stock), value = TRUE))
  nm_fac_stock[is.na(get(v)), (v) := 0L]

nm_fac_stock[, tank_closure_clean := as.integer(
  any_closure == 1L & tank_closure_revealed == 0L & tank_closure_known_leak == 0L
)]
nm_fac_stock[, `:=`(
  lust_within_90d_of_closure = tank_closure_revealed_wide,
...
nm_installs_for_lust <- nm_tanks[
  !is.na(tank_installed_date),
  .(panel_id, tank_install_date = tank_installed_date)
]
setkey(nm_installs_for_lust, panel_id)

nm_leaks_install_pairs <- nm_leaks_for_lust[nm_installs_for_lust,
                                             on = .(panel_id),
                                             allow.cartesian = TRUE,
                                             nomatch = NULL]
nm_leaks_install_pairs <- nm_leaks_install_pairs[!is.na(report_date) &
                                                    !is.na(tank_install_date)]
nm_leaks_install_pairs[, days_after_install :=
                          as.numeric(report_date - tank_install_date)]
nm_leaks_install_pairs[, `:=`(
  is_install_prim = days_after_install >= 0 & days_after_install <= 60,
  is_install_nar  = days_after_install >= 0 & days_after_install <= 30,
  is_install_wid  = days_after_install >= 0 & days_after_install <= 90,
  is_install_reg  = days_after_install >= 0 & days_after_install <= 45
...
nm_leak_install_flags <- nm_leaks_install_pairs[, .(
  any_install_prim = as.integer(any(is_install_prim, na.rm = TRUE)),
  any_install_nar  = as.integer(any(is_install_nar,  na.rm = TRUE)),
  any_install_wid  = as.integer(any(is_install_wid,  na.rm = TRUE)),
  any_install_reg  = as.integer(any(is_install_reg,  na.rm = TRUE))
...
nm_fac_stock <- merge(nm_fac_stock, nm_leak_install_flags,
                      by = c("panel_id", "panel_year"), all.x = TRUE)
for (v in c("any_install_prim", "any_install_nar",
            "any_install_wid", "any_install_reg")) {
  nm_fac_stock[is.na(get(v)), (v) := 0L]
}
nm_fac_stock[, leak_first_year_churn := any_install_prim]

rm(nm_lust_pairs, nm_closure_lust_flags, nm_annual_lust_flags,
   nm_closures_for_lust, nm_leaks_for_lust,
   nm_installs_for_lust, nm_leaks_install_pairs, nm_leak_install_flags)
...
nm_fac_stock[, event_first_leak := as.integer(leak_year == 1L &
                                               has_previous_leak == 0L)]

log_step(sprintf("NM leak first-year-churn flags (0-60d): %s of %s leak-fac-years (%.1f%%)",
  fmt_n(nm_fac_stock[leak_year == 1L & leak_first_year_churn == 1L, .N]),
  fmt_n(nm_fac_stock[leak_year == 1L, .N]),
  100 * mean(nm_fac_stock[leak_year == 1L, leak_first_year_churn == 1L])))

# TX-FR and control-FR fee columns do not apply to NM. Same treatment as PA
# (D4/ticket 050 PART A): leave the fee NA + flagged -- do not invent the NM
# fee (likely gas-tax funded, resolved separately).
nm_fac_stock[, `:=`(
  uses_private_dec = "None", uses_self_dec = "None",
  fr_covered_dec   = "None", DETAIL_TYPE_dec = "None", CATEGORY_dec = "None",
  months_covered    = 0L, months_with_data = 0L,
...
#### NM.13 NM final facility panel assembly (MIRRORS T046.13) ####
cat("NM.13: NM final facility panel assembly...\n")

nm_facility_panel <- merge(
  nm_fac_stock,
  nm_fac_bio[, .(panel_id, first_install_yr, last_install_yr,
              first_install_date, last_install_date,
              total_tanks_ever, total_tanks_closed_ever, total_capacity_ever,
              max_capacity_ever, mean_capacity_ever,
              total_sw_ever, total_dw_ever,
...
nm_facility_panel <- merge(
  nm_facility_panel,
  nm_fac_mm_reform[, .(panel_id, make_model_fac,
    fac_wall_reform, fac_fuel_reform, oldest_age_bin,
    n_tanks_at_reform, n_sw_at_reform, n_dw_at_reform,
    total_capacity_reform, oldest_age_reform,
    any_sw_at_reform, any_dw_at_reform,
...
nm_facility_panel[, yrs_since_first_install := as.integer(panel_year) - first_install_yr]
nm_facility_panel[, wall_type := fcase(
  has_double_walled_dec == 1L & has_single_walled_dec == 0L, "Double-Walled",
  has_single_walled_dec == 1L & has_double_walled_dec == 0L, "Single-Walled",
  has_double_walled_dec == 1L & has_single_walled_dec == 1L, "Mixed",
  default = "Unknown"
)]
nm_facility_panel[, wall_pure := fcase(
  wall_type == "Single-Walled", "Single-Walled",
  wall_type == "Double-Walled", "Double-Walled",
  default = NA_character_
)]
nm_facility_panel[, is_pure_wall := as.integer(!is.na(wall_pure))]
nm_facility_panel[, age_bins := cut(avg_tank_age_dec,
  breaks = c(0, 3, 6, 9, 12, 15, 18, 21, 24, Inf),
  include.lowest = TRUE,
  labels = c("0-2", "3-5", "6-8", "9-11", "12-14", "15-17", "18-20", "21-23", "24+")
)]
...
log_step(sprintf("NM facility panel: %s rows | %s fac | exits=%s | complete_closures=%s | entries=%s",
  fmt_n(nrow(nm_facility_panel)),
  fmt_n(uniqueN(nm_facility_panel$panel_id)),
  fmt_n(nm_facility_panel[facility_exit == 1L, .N]),
  fmt_n(nm_facility_panel[facility_complete_closure == 1L, .N]),
  fmt_n(nm_facility_panel[facility_entry == 1L, .N])))

n_nm_unknown_wall_share <- mean(nm_facility_panel$has_unknown_wall == 1L, na.rm = TRUE)
log_step(sprintf("NM unknown-wall share (facility-years): %.3f (expect ~1.0)",
  n_nm_unknown_wall_share))
cat("\n")
...
log_step("Flags set: study rows (study, 1, 1); PA/NM rows (pricing_only, 0, 1).")
cat("\n")
...
coerce_shared_cols(panel_dt_main, nm_panel_dt)
assert_cols_match(panel_dt_main, pa_panel_dt, "panel_dt_main", "pa_panel_dt")
assert_cols_match(panel_dt_main, nm_panel_dt, "panel_dt_main", "nm_panel_dt")
panel_dt_final <- rbindlist(list(panel_dt_main, pa_panel_dt, nm_panel_dt), use.names = TRUE)
log_step(sprintf("panel_dt: %s study rows + %s PA rows + %s NM rows = %s total",
  fmt_n(nrow(panel_dt_main)), fmt_n(nrow(pa_panel_dt)), fmt_n(nrow(nm_panel_dt)),
  fmt_n(nrow(panel_dt_final))))

coerce_shared_cols(facility_panel_main, pa_facility_panel)
coerce_shared_cols(facility_panel_main, nm_facility_panel)
assert_cols_match(facility_panel_main, pa_facility_panel, "facility_panel_main", "pa_facility_panel")
assert_cols_match(facility_panel_main, nm_facility_panel, "facility_panel_main", "nm_facility_panel")
facility_panel_final <- rbindlist(list(facility_panel_main, pa_facility_panel, nm_facility_panel), use.names = TRUE)
log_step(sprintf("facility_panel: %s study rows + %s PA rows + %s NM rows = %s total",
  fmt_n(nrow(facility_panel_main)), fmt_n(nrow(pa_facility_panel)), fmt_n(nrow(nm_facility_panel)),
  fmt_n(nrow(facility_panel_final))))
cat("\n")
...
log_step(sprintf("NM tank-years added: %s", fmt_n(nrow(nm_panel_dt))))
log_step(sprintf("NM facility-years added: %s", fmt_n(nrow(nm_facility_panel))))
log_step(sprintf("NM leak fac-years: %s | leak reports: %s | leak incidents: %s",
  fmt_n(nm_facility_panel[leak_year == 1L, .N]),
  fmt_n(nm_facility_panel[, sum(n_leak_reports)]),
  fmt_n(nm_facility_panel[, sum(n_leak_incidents)])))
log_step(sprintf("NM unknown-wall share (facility-years): %.3f (expect ~1.0)",
  mean(nm_facility_panel$has_unknown_wall == 1L, na.rm = TRUE)))
log_step(sprintf("NM completeness: fuel known %.1f%% | capacity known %.1f%% | install_cohort known %.1f%%",
  nm_pct_fuel_known, nm_pct_cap_known, nm_pct_cohort_known))
