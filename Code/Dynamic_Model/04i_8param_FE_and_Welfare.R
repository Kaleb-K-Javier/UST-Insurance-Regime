...
profile_neg_loglik_8p_fe <- local({
  ...
  function(theta_raw, cache, config_8p_fe) {
    ...
    if (!isTRUE(eq$converged)) return(1e10)
...
SE  <- if (anyNA(H)) rep(NA_real_, n_p) else {
  Vinv <- solve(H)
  dv <- diag(Vinv); sqrt(ifelse(dv > 0, dv, NA_real_))
}
...
# Per-cell table is in model units (no E in delta_V). For CF B specifically we
# pick the HEALTH_ONLY equilibrium so the comparison point is unambiguous.
cf_B_per_cell <- cf_B_by_E[["HEALTH_ONLY"]]
welfare_long <- rbindlist(list(
  build_cell_welfare(eq_base, cf_A$eq,         cf_A$label),
  build_cell_welfare(eq_base, cf_B_per_cell$eq, cf_B_per_cell$label),
  build_cell_welfare(eq_base, cf_C$eq,         cf_C$label),
  build_cell_welfare(eq_base, cf_D$eq,         cf_D$label)
))
writeLines(
  "# CF B per-cell comparison uses cf_B at E = HEALTH_ONLY ($17K, Marcus 2021 SVI)",
  file.path(OUT_TAB, "04i_Welfare_PerCell.csv"))
fwrite(welfare_long, file.path(OUT_TAB, "04i_Welfare_PerCell.csv"), append = TRUE,
       col.names = TRUE)
