...
\textbf{Underground Storage Tanks (USTs) are the largest source of
groundwater contamination in the U.S.}

\begin{itemize}
\tightlist
\item
  \(\sim 500{,}000\) tanks beneath the country's gas stations; a
  confirmed release becomes a \textbf{Leaking UST (LUST)} event with
  mean cleanup \(\sim\$420{,}000\) (EPA, 2024)
\item
  Federal law mandates liability insurance to internalize remediation
  costs. Shavell (1982): strict liability disciplines owners \emph{only}
  when \(P_i \propto \text{Risk}_i\)
\item
  \textbf{38 states charge a uniform premium} \(\bar P\) regardless of
  age, wall type, or fuel --- zero marginal cost of aging on the
  operator's bottom line
\item
  Upgrading wall construction is irreversible and expensive: single
  \(\to\) double-wall requires full tank replacement
  (\(\sim 60\)--\(90\%\) premium; GAO, 1987). Operators face a price
  signal \emph{and} a sticky technology
\end{itemize}

\vfill

\textbf{Research Question.} Does replacing uniform-premium public
insurance with risk-based private coverage induce firms to internalize
...
\begin{frame}{Appendix: LUST Event Study --- Decomposition}
\phantomsection\label{appendix-lust-event-study-decomposition}
\BerkeleyLightMode
