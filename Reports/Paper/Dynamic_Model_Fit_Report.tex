...
replacement model under three nested specifications, each estimated by
...
\item
  ...
  price/risk slopes.
\item
  \textbf{8-parameter + state fixed effects} --- the 8p structural
  specification augmented with 17 state-level intercepts \(\alpha_g\)
  that enter the measurement equation (not the agent's equilibrium) for
  the 17 control states; Texas is the baseline
  (\(\alpha_{\text{TX}} \equiv 0\)).
\end{enumerate}

The report is organized for direct visual and statistical comparison:
Section~\ref{sec-params} reports the three parameter tables and the FE
alphas; Section~\ref{sec-comparison} reports the three-way
information-criteria and likelihood-ratio table; Section~\ref{sec-fit}
reports model-implied vs.~empirical action shares at the
...
\texttt{Code/Dynamic\_Model/04k\_Fit\_Report\_Artifacts.R} from saved
fit objects under \texttt{Output/Estimation\_Results/}. No estimation is
...
\subsection{8-parameter + state fixed effects}\label{sec-params-8pfe}

Table~\ref{tbl-theta-8pfe} reports the 8-structural-parameter block from
the joint \((\theta, \alpha)\) estimation. The 17 state fixed effects
are reported separately in Table~\ref{tbl-fe-alphas}.
...
and nested likelihood-ratio tests across the three specifications. All
three are fit on the same observed sample, so the comparison is direct.

\begin{table}

\caption{\label{tbl-comparison}Three-way nested model comparison. AIC
\(= -2\log L + 2k\); BIC \(= -2\log L + k\log N\). Likelihood-ratio
tests: 8p vs 4p (\(\Delta k = 4\)), 8p+FE vs 8p (\(\Delta k = 17\)).
Both LR statistics are far in the tail of their reference \(\chi^2\)
distributions, with \(p\)-values numerically zero.}
...
BIC agree on the ranking 4p \(\succ\) 8p \(\succ\) 8p+FE (lower is
better), with the BIC penalty large enough to make 8p+FE still preferred
despite the 17 additional parameters.
...
\subsection{8-parameter + FE fit}\label{sec-fit-8pfe}
...
\caption{\label{tbl-fitq-8pfe}8-parameter + FE weighted-RMSE summary by
wall, regime, and action. Same construction as Table~\ref{tbl-fitq-4p}.}
...
\subsection{8-parameter + FE}\label{sec-appendix-8pfe}

\begingroup\fontsize{8}{10}\selectfont

\begin{longtable}{lllrrrrrrrrrr}

\caption{\label{tbl-appendix-8pfe}8-parameter + FE per-cell fit. Same
construction as Table~\ref{tbl-appendix-4p}.}
