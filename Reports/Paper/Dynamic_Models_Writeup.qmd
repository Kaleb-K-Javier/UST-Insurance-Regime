...
L <- 10000        # Cleanup cost (Private internalization target)
...
regimes <- list(
  F = list(
    ...
    D = 0.1 * L                   # 10% deductible
  ),
  S = list(
    name = "Self-Insurance",
    p_func = function(a, y) 0,   # No premium
    D = L                         # Facility pays Cleanup (L) but not Health (H)
  ),
  R = list(
    name = "Risk-Rated Private",
    p_func = function(a, y) (1 + lambda) * y * L,
    D = 0.25 * L                  # 25% deductible
  ),
  SOC = list(
    name = "Social Optimum",
    p_func = function(a, y) 0,
    # SOC Internalizes Cleanup (L) + External Health Damages (H(a))
    # We calibrate H(a) approx equal to L, so S/R internalize ~50% of total
    D = function(a) L + (L * (1 + 0.02 * a))
  )
)

# Solver function
solve_regime <- function(regname, params) {
  Amax <- 90      # EXTENDED time horizon to 90 years
  ages <- 0:Amax
  n <- length(ages)
  ...
  # Deductible/Damage Vector
  if (is.function(params$D)) {
    D_vec <- sapply(ages, params$D)
  } else {
    D_vec <- rep(params$D, n)
  }

  # Initialize value arrays
  W_SW <- numeric(n + 1); W_DW <- numeric(n + 1)
  W_SW[n+1] <- -k; W_DW[n+1] <- -k
  ...
  list(name = params$name, regname = regname, ages = ages, B = B, a_star = a_star)
}

# Run solver and combine
...
DT <- rbindlist(lapply(results, function(res) {
  data.table(regime = res$name, age = res$ages, B = res$B, a_star = res$a_star)
...
**Interpretation:** @fig-toy-benefit shows the equivalent benefit curves $B(a)$ for all regimes. The vertical distance between each curve and the dashed retrofit cost line represents the net benefit of upgrading at each age. Risk-rated insurance (green) creates the largest benefits at younger ages, inducing retrofit at `r summary_table[regime == "R", a_star]` years. Self-insurance (blue) yields intermediate timing at `r summary_table[regime == "S", a_star]` years. Flat-fee pooling (red) delays retrofit until `r summary_table[regime == "F", a_star]` years due to minimal private internalization. The social optimum (purple) occurs even earlier at `r summary_table[regime == "SOC", a_star]` years, reflecting external damages not captured by private contracts.
